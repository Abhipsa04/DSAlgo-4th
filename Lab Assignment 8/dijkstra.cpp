...
void print(int dist[],int pred[]){
	cout<<"Vertex \t\t Distance from Source"<<endl;
    for (int i = 1; i < V; i++){
           cout<<char(pred[i]+65)<<"-"<<char(i+65)<<"\t\t"<<dist[i]<<endl;
           graph_res[pred[i]][i]=dist[i];
           graph_res[i][pred[i]]=dist[i];
    }
...
int main(){
	fstream infile;
	infile.open("graphdij.txt" , ios::in);
	if(!infile)
	{
